...
{
	if (Robot.Xbox.XboxReceiverConnected)
	{
		for (uint8_t i = 0; i < 4; i++)
		{
			if (Robot.Xbox.Xbox360Connected[i])
			{
				//L2 Trigger
				if (Robot.Xbox.getButtonPress(R2, i))
				{
					Robot.State.LiftSpeed = Robot.Xbox.getButtonPress(R2, i) * 1;
				}
				//R2 Trigger
				else if (Robot.Xbox.getButtonPress(L2, i))
				{
					Robot.State.LiftSpeed = Robot.Xbox.getButtonPress(L2, i) * -1;
				}
				else
				{
				...
				if (Robot.Xbox.getAnalogHat(LeftHatY, i) > 7500)
				{
					//Going backwards
					Robot.State.LeftSpeed = map(Robot.Xbox.getAnalogHat(LeftHatY, i), 7500, 32767, 0, 255);
				}
				else if (Robot.Xbox.getAnalogHat(LeftHatY, i) < -7500)
				{
					//going forwards
					Robot.State.LeftSpeed = map(Robot.Xbox.getAnalogHat(LeftHatY, i), -7500, -32767, 0, -255);
				}
				...
				if (Robot.Xbox.getAnalogHat(RightHatY, i) > 7500)
				{
					Robot.State.RightSpeed = map(Robot.Xbox.getAnalogHat(RightHatY, i), 7500, 32767, 0, 255);
				}
				else if (Robot.Xbox.getAnalogHat(RightHatY, i) < -7500)
				{
					Robot.State.RightSpeed = map(Robot.Xbox.getAnalogHat(RightHatY, i), -7500, -32767, 0, -255);
				}
				else
				...
				if (Robot.Xbox.getButtonClick(Y, i))
				{
					Robot.State.LimitLift = !Robot.State.LimitLift;
				}

				if (Robot.Xbox.getButtonClick(UP, i))
				{
